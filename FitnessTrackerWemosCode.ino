...
This code is for the ENGR 7B Fitness Tracker Project

Team Name: WiiFit
Team Members: Brady Cason, Janice Ho, Sofia Fowler, Sofia Poulis, Tyler Leong
Date of Last Edit: 3/13/24

Summary:
-Count Steps from Accelerometer
    -Calculate steady state values of Accelerometer
-Count Flights Climbed from Barometer
-Input/ Output:
    -Display data on OLED Screen
    -Send steps and flights to Blynk App
    -Recieve steps goal and flights goal from Blynk App
    -Button (Controls OLED and resets program)
    -Piezo Buzzer (Plays wii sports song)
...
void getBarometerData()
{
  ...
  if (altitudeDif >= landingHeight - 0.15)
  {
    ...
    Blynk.virtualWrite(V1, flightsClimbed);
